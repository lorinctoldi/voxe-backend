...
router.post('/scrape/hasznaltauto', async (req, res) => {
  ...
  if(!makeId || !modellId) return res.json('No make or no model provided!');
...
router.post('/scrape/mobile', async (req, res) => {
  ...
  const { makeId, modellId, design, startYear, endYear } = req.body;
  if(!makeId || !modellId) return res.json('No make or no model provided!');
  if(startYear > endYear) return res.json('Starting year is higher than ending year!')

  try {
    const scrapedData = await mobile_scraper.scrapeAllData(makeId, modellId, design, startYear, endYear);
    res.json(scrapedData);
  } catch (error) {
    console.error('Error during scraping:', error);
    res.status(500).json({ error: 'An error occurred during scraping.' });
